...
test("hardening: a stale success must not cut short the latest search debounce", async (t) => {
  ...
  assert.equal(h.state.rows.length, 0, "do not render the superseded result");
...
test("hardening: current snapshot failures still perform one refresh and a fresh first-page query", async (t) => {
  const h = await harness(t);
  const all = prepareRows(h);
  let calls = 0;
  h.respondWith((request) => {
    if (++calls === 1) throw new Error("快照已变化，请重新校验");
    return pageResult(request, all);
  });
  await h.runQuery();
  await settle();
  assert.equal(schemaCalls(h).length, 2);
  assert.deepEqual(h.queries.map((request) => request.page), [1, 1]);
  assert.equal(h.state.loading, false);
  assert.equal(h.state.rows.length, 1);
});
...
async function harness(t, saved) {
  ...
  let nextTimer = 0, nextId = 0, queryHandler, schemaHandler, deleteHandler;
  const observers = [];
