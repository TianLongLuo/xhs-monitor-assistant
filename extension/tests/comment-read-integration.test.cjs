...
function harness(result = sample) {
  ...
  const context = vm.createContext({
    console, Promise, Date, Map, Set, isCommentLocatorSurface: () => false,
...
test("unchanged-but-incomplete audit is saved without displaying 最新 or looping retries", async () => {
  ...
  for (const [complete, muted] of [[false, false], [true, false], [false, true]]) {
    ...
    const toasts = [];
    let syncs = 0;
    const context = vm.createContext({ isCommentLocatorSurface: () => false, processPanel: panel, PROCESS_PANEL_CLASS: "fixture",
