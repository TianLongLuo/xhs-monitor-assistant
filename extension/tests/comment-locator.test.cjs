...
const ID = "aaaaaaaaaaaaaaaaaaaa0002";
const PARENT = "aaaaaaaaaaaaaaaaaaaa0001";
const OTHER = "aaaaaaaaaaaaaaaaaaaa0003";
...
test("an unloaded parent uses top-level loaders, then expands only the newly loaded target thread", async () => {
  ...
  const promise = run(h, { commentId: ID, parentCommentId: PARENT }); await h.advance(80);
...
test("a virtualized list rewinds at most once and stops before a whole-list traversal", async () => {
  ...
  const promise = run(h, { commentId: ID }); await h.advance(80);
...
test("native reply pagination may advance immediately after actual thread progress", async () => {
  ...
  const promise = run(h, { commentId: ID, parentCommentId: PARENT }); await h.advance(160);
  assert.equal((await promise).ok, true); assert.deepEqual(clicks(h).map(action => action.at), [0, 80, 160]);
  h.view.dispatchEvent({ type: "pagehide" }); h.assertIdle();
});
...
test("freeze gate: canonical DB target traverses actual worker -> content -> locator against prefixed DOM ID", async () => {
  const h = fixture(); const noteId = "aaaaaaaaaaaaaaaaaaaa0004";
