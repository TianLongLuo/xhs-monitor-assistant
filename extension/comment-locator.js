(function (root, factory) {
  ...
  function identities(node, ctx) {
    ...
    function read(carrier) {
      for (const name of ["data-comment-id", "comment-id"]) add(attr(carrier, name));
      for (const name of ["data-id", "id"]) {
        const value = attr(carrier, name);
        if (!result.size || /^(?:comment-)?[\da-fA-F]{24}$/.test(value)) add(value);
      }
    }
    read(node);
  ...
  function matchInContext(ctx, comment) {
    ...
    if (!id.startsWith("dom-")) {
      ...
      if (matching.length) return miss("conflicting_id");
      // DOM variants may omit native IDs. Only a unique complete body+author
      // with no contradictory ID is eligible; never substring/fuzzy matching.
      if (!/^[\da-fA-F]{24}$/.test(id) && !id.startsWith("legacy-")) return miss();
    }
    ...
    const exact = id.startsWith("dom-") ? rows.filter((node) => hasIdentity(node, id, ctx) && consistent(node)) : [];
  ...
  function locate(options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      function found(match) {
        ...
        highlightTimer = setTimer(close, Number.isFinite(options.highlightMs) ? Math.max(6000, Math.min(120000, options.highlightMs)) : 6000);
      ...
      function watch() {
        if (!check()) return;
        // Native rerenders may replace an already-found row. Transfer only a
        // freshly verified match, without restarting loading or scrolling.
        if (settled && highlighted && !within(root, highlighted)) {
          try {
            const replacement = matchInContext(context(root, noteId, utils), comment).element;
            if (replacement && check()) {
              if (highlights.get(highlighted) === token) {
                highlights.delete(highlighted); highlighted.classList?.remove(HIGHLIGHT);
              }
              highlighted = replacement; highlights.set(replacement, token);
              replacement.classList?.add(HIGHLIGHT);
            }
          } catch (_error) { /* Keep the read-only watch bounded by highlightTimer. */ }
        }
